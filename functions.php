<?php
/**
 * right functions and definitions
 *
...
if ( ! function_exists( 'right_setup' ) ) :
	/**
	...
	function right_setup() {
		 ...
		 * If you're building a theme based on right, use a find and replace
		 * to change 'right' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'right', get_template_directory() . '/languages' );
...
function right_widgets_init() {
	register_sidebar(
		array(
			...
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'right' ),
 ...
 */
function right_scripts() {
	wp_enqueue_style( 'right-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'right-style', 'rtl', 'replace' );

	wp_enqueue_script( 'right-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'right_scripts' );
...
add_image_size('thumb-150', 150, 150, true); // добавляем еще один размер картинкам 94x94 с обрезкой
add_image_size('lic-207x285', 207, 285, true);
add_image_size('place-270x152', 270, 152, true);
add_image_size('offer-320x135', 320, 135, true);
